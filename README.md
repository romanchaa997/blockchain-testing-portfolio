...
В проекте реализован контракт **UserStorage**, который позволяет каждому пользователю сохранять своё значение независимо и генерирует событие при обновлении данных.

### Основные функции:
- `set(uint256 _data)`: Сохраняет значение для вызывающего пользователя и генерирует событие `DataUpdated`.
- `get()`: Возвращает значение, установленное вызывающим пользователем.
- `getFor(address _user)`: Позволяет получить значение для указанного адреса.

### Тест-кейсы:
- **test_self_set_get:** Проверяет, что пользователь может установить и получить своё значение.
- **test_independent_storage:** Проверяет, что разные пользователи хранят свои значения независимо.
- **test_event_emission_user:** Проверяет, что при вызове функции `set()` генерируется событие `DataUpdated` с правильными параметрами.
